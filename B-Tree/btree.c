...
void bt_split_child(btNode x, int pos, bTree *tree, FILE *fp, int split_root){
    ...
    for(int j = 0; j <= t && (j+t+1)<= y.numKeys-1; j++){ // move elements to new node
        ...
        y.keys[j+t+1].codigo = -1;                         // erase the element from the previous node
    ...
    y.keys[t].codigo = -1;                                  // erase the updated element from the previous node
    y.keys[t].salario = -1;
...
btNode bt_insert_nonfull(btNode node, element key, bTree *tree, FILE *fp){
    ...
    if(node.isLeaf == 1){                                      // if in a leaf insert the new element
        int i = pos-1;
        while(i >= 0 && key.codigo < node.keys[i].codigo){           // find the correct position
        ...
        int i = pos-1;
        while (key.codigo < node.keys[i].codigo && i >= 0) {         // get the correct child of the node
            i--;
            pos--;
        }
        ...
        if(x.numKeys == tree->order-1){                        // is this child full?
            ...
            if(key.codigo > x1.keys[pos].codigo)                     // adjust the position if needed
                pos++;
        }
        btNode x1 = disk_read(n_pd, tree->order, fp);          // get the updated node
...
void bt_merge_children(btNode node, int pos, int order, FILE *fp){
    ...
    node.keys[pos].codigo = -1;                           // delete item the item borrowed
...
void bt_borrow_from_left_sibling(btNode node, int pos, int order, FILE *fp){
    ...
    z.keys[z.numKeys-1].codigo = -1;                       // remove borrowed item
...
int btSearch(btNode node, int order, element key, FILE *fp){
    ...
    int pos = 0;
    while(pos < node.numKeys && key.codigo > node.keys[pos].codigo){  // find the correct position
        pos++;
    }
    if(pos <= node.numKeys && key.codigo == node.keys[pos].codigo){   // is the item one of the key`s of this node?
